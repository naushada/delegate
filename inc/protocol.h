...
namespace mna {
  ...
  class FSM {
    ...
    public:
      /* Type Aliasing */
		  using receive_delegate_t = delegate<int32_t (const char*, uint32_t)>;
		  using state_delegate_t = delegate<void ()>;

      FSM()
			{
        on_receive.reset(); on_exit.reset(); on_entry.reset();
			}
      ...
      /**
			 ...
			 * @return
			 * */
      int32_t rx(const char *inPtr, uint32_t inLen) const
			{
				return(on_receive(inPtr, inLen));
			}
  ...
  /**
    ...
    class OnDiscover {
        ...
        /**
				 ...
				 * */
				int32_t receive(const char *inPtr, uint32_t inLen)
				{
    ...
    class dhcpEntry {
        ...
        dhcpEntry(uint32_t clientIP, uint32_t xid, uint32_t routerIP, uint32_t dnsIP,
        ...
        int32_t rx(const char *in, uint32_t inLen)
				{
				  ...
				  int32_t ret = getState().rx(in, inLen);
    ...
    using dhcp_entry_onIP_t = std::unordered_map<uint32_t, dhcpEntry>;
        ...
        int32_t rx(const char *in, uint32_t inLen)
          ...
          it = m_dhcpUmapOnMAC.find(MAC);
            ...
            dhcpEntry dEnt(123, m_xid, m_routerIP, m_dnsIP, m_lease, m_mtu, m_serverID, m_domainName);
